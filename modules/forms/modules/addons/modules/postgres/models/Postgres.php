...
/**
 * This is the model class for table "{{%addon_postgres}}".
 *
 * @property integer $id
 ...
 *
 * @property Form $form
 * @property PostgresField[] $items
 * @property User $author
 * @property User $lastEditor
 */
class Postgres extends \yii\db\ActiveRecord
{
    ...
    {
        return [
            [['name', 'form_id', 'db_host', 'db_port', 'db_name', 'db_user', 'db_schema', 'db_table'], 'required'],
            [['form_id', 'status', 'event', 'db_port', 'created_by', 'updated_by', 'created_at', 'updated_at'], 'integer'],
            [['name', 'db_host', 'db_name', 'db_user', 'db_pass', 'db_schema', 'db_table'], 'string', 'max' => 255],
            [['conditions'], 'string'],
        ];
    }
    ...
    {
        return $this->hasMany(PostgresField::class, ['postgres_id' => 'id']);
    }
    ...
    public function beforeDelete()
    {
        if (parent::beforeDelete()) {
            PostgresField::deleteAll(["postgres_id" => $this->id]);
            return true;
        } else {
            return false;
        }
