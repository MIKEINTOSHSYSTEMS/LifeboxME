...
namespace app\modules\addons\modules\postgres\controllers;

use app\components\User;
use app\helpers\ArrayHelper;
use app\helpers\SubmissionHelper;
use app\models\Form;
use app\modules\addons\modules\postgres\models\Postgres;
use app\modules\addons\modules\postgres\models\PostgresField;
use app\modules\addons\modules\postgres\models\PostgresSearch;
use app\modules\addons\modules\postgres\services\PostgresService;
use Exception;
use Yii;
...
/**
 * AdminController implements the CRUD actions for Postgres model.
 */
class AdminController extends Controller
{
                ...
                'rules' => [
                    ...
                    ['actions' => ['create'], 'allow' => true, 'matchCallback' => function ($rule, $action) {
                        if (Yii::$app->request->isGet && Yii::$app->user->can('configureFormsWithAddons', ['listing' => true])) {
                            return true;
                        } else if ($postData = Yii::$app->request->post('Postgres')) {
    ...
    public function actions()
    {
        return [
            'delete-multiple' => [
                ...
                'modelClass' => 'app\modules\addons\modules\postgres\models\Postgres',
            ...
            'list-fields' => [
                ...
                'outputCallback' => function ($db_table, $params) {
                    ...
                    $db_pass = null;
                    $db_schema = null;
                    $model = Postgres::findOne(['db_table' => $db_table]);
                    if (!empty($model->form)) {
                        ...
                        $db_host = $session['postgres_db_host'];
                        $db_port = $session['postgres_db_port'];
                        $db_name = $session['postgres_db_name'];
                        $db_user = $session['postgres_db_user'];
                        $db_pass = $session['postgres_db_pass'];
                        $db_schema = $session['postgres_db_schema'];
                    }

                    if ($db_host && $db_port && $db_name && $db_user && $db_schema) {
    ...
    /**
     * Lists all Postgres models.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new PostgresSearch();
    ...
    /**
     * Displays a single Postgres model.
     * @param $id
     * @return string
    ...
    public function actionCreate()
    {
        ...
        if ($request->isPost && $model->load($request->post())) {
            try {
                $session = Yii::$app->session;
                $session['postgres_db_host'] = $model->db_host;
                $session['postgres_db_port'] = $model->db_port;
                $session['postgres_db_name'] = $model->db_name;
                $session['postgres_db_user'] = $model->db_user;
                $session['postgres_db_pass'] = $model->db_pass;
                $session['postgres_db_schema'] = $model->db_schema;
                ...
                if ($connected && $model->validate()) {
                    $model->items = Yii::$app->request->post('PostgresField', []);
                    $model->save(false);
                    Yii::$app->getSession()->setFlash(
                        'success',
                        Yii::t('app', 'The PostgreSQL integration has been successfully configured.')
                    );
    ...
    /**
     * Updates an existing Postgres model.
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load($request->post()) && $model->validate()) {
            $items = Yii::$app->request->post('PostgresField', []);
            if (!empty($items)) {
                $model->items = $items;
            }
            $model->save();
    ...
    /**
     * Enable / Disable multiple Postgres models
     *
     * @param $status
    ...
    {
        $models = Postgres::findAll(['id' => Yii::$app->getRequest()->post('ids')]);
    ...
    /**
     * Deletes an existing Postgres model.
    ...
    /**
     * Finds the Postgres model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Postgres the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = Postgres::findOne(['id' => $id])) !== null) {
            return $model;
        } else {
