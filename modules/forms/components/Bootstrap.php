    ...
    public function bootstrap($app)
    {
        ...
        $app->on(Application::EVENT_BEFORE_REQUEST, function () use ($app) {
            ...
            try {
                ...
                if ($userModule) {
                    ...
                    if (isset($userModule->mailParams)) {
                        $userModule->mailParams = [
                            'fromEmail' => MailHelper::from($app->settings->get("app.supportEmail")),
                            'welcomeMailSubject' => Yii::t('app', 'Welcome to {0}', $app->settings->get("app.name")),
                            'confirmationMailSubject' => Yii::t('app', 'Confirm account on {0}', $app->settings->get("app.name")),
                            'reconfirmationMailSubject' => Yii::t('app', 'Confirm email change on {0}', $app->settings->get("app.name")),
                            'recoveryMailSubject' => Yii::t('app', 'Complete password reset on {0}', $app->settings->get("app.name")),
                        ];
                    }
                }
                ...
                if ($apiModule && $restApiStatus = (int) $app->settings->get('restApi', 'app', 0)) {
                    if ($restApiStatus > 0) {
        ...
        // After form submission is created
        $app->on(
            FormSubmission::EVENT_SUBMISSION_CREATED,
            ['app\events\handlers\SubmissionEventHandler', 'onSubmissionCreated']
        );

        // After form submission is updated
        $app->on(
            FormSubmission::EVENT_SUBMISSION_UPDATED,
            ['app\events\handlers\SubmissionEventHandler', 'onSubmissionUpdated']
        );
