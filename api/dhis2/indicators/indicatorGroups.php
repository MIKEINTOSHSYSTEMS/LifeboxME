...
// program_indicatorGroups.php

/**
 * Fetches DHIS2 program indicatorGroups and returns their id and displayName in a structured format
...
// Main execution
try {
    // Fetch program indicatorGroups data
    $indicatorGroups = fetchDhis2Data('api/indicatorGroups.json?paging=false');

    // Extract only 'id' and 'displayName' from the program indicatorGroups
    if (isset($indicatorGroups['indicatorGroups'])) {
        ...
        throw new Exception("No program indicatorGroups found in the API response.");
    }
} catch (Exception $e) {
    // Error handling
    header('Content-Type: application/json; charset=utf-8');
