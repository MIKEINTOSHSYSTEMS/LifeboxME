...
/**
 * Fetches DHIS2 dataItems and returns their id and displayName in a structured format
...
// Main execution
try {
    // Fetch program dataItems data
    $dataItems = fetchDhis2Data('api/dataItems.json?paging=false');

    // Extract only 'id' and 'displayName' from the program dataItems
    if (isset($dataItems['dataItems'])) {
        ...
        throw new Exception("No program dataItems found in the API response.");
    }
} catch (Exception $e) {
    // Error handling
    header('Content-Type: application/json; charset=utf-8');
