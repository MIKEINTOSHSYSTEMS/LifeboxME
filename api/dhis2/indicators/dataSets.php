...
// Main execution
try {
    // Fetch program dataSets data
    $dataSets = fetchDhis2Data('api/dataSets.json?paging=false');

    // Extract only 'id' and 'displayName' from the program dataSets
    if (isset($dataSets['dataSets'])) {
        ...
        throw new Exception("No program dataSets found in the API response.");
    }
} catch (Exception $e) {
    // Error handling
    header('Content-Type: application/json; charset=utf-8');
