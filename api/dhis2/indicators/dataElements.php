...
// Main execution
try {
    // Fetch program dataElements data
    $dataElements = fetchDhis2Data('api/dataElements.json?paging=false');

    // Extract only 'id' and 'displayName' from the program dataElements
    if (isset($dataElements['dataElements'])) {
        ...
        throw new Exception("No program dataElements found in the API response.");
    }
} catch (Exception $e) {
    // Error handling
    header('Content-Type: application/json; charset=utf-8');
