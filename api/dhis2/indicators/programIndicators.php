...
// program_indicators.php

/**
 * Fetches DHIS2 program indicators and returns their id and displayName in a structured format
...
// Main execution
try {
    // Fetch program indicators data
    $programIndicators = fetchDhis2Data('api/programIndicators.json?paging=false');

    // Extract only 'id' and 'displayName' from the program indicators
    if (isset($programIndicators['programIndicators'])) {
        ...
        throw new Exception("No program indicators found in the API response.");
    }
} catch (Exception $e) {
    // Error handling
    header('Content-Type: application/json; charset=utf-8');
