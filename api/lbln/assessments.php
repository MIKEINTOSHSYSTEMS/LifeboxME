<?php
// assessments.php - Standalone Lifebox Assessments API
...
function getAssessmentResponses($assessmentId, $filters = [], $maxPages = null) {
    return fetchAllPaginatedData('/assessments/' . urlencode($assessmentId) . '/responses', $filters, $maxPages);
}

// Get single page of assessment responses
function getAssessmentResponsesSinglePage($assessmentId, $page = 1, $itemsPerPage = null) {
    $config = loadEnv();
    ...
    $queryString = http_build_query($queryParams);
    $url = $baseUrl . '/assessments/' . urlencode($assessmentId) . '/responses?' . $queryString;
...
// Filter assessment responses
function filterAssessmentResponses($responses, $filters) {
...
try {
    ...
    if (!$assessmentId) {
        http_response_code(400);
        echo formatResponse(['error' => 'Assessment ID is required']);
        exit;
    }
    ...
    // Get assessment responses
    if ($singlePage) {
        $result = getAssessmentResponsesSinglePage($assessmentId, $page, $itemsPerPage);
    } else {
        $result = getAssessmentResponses($assessmentId, $apiFilters, $maxPages);
    }
    ...
    if (!isset($result['error']) && isset($result['data']) && is_array($result['data'])) {
        ...
        if (!empty($clientFilters)) {
            $filteredResponses = filterAssessmentResponses($result['data'], $clientFilters);
