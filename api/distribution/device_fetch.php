...
<head>
    ...
    <style>
        <?php include __DIR__ . '/assets/styles/style.css'; ?><?php include __DIR__ . '/assets/styles/fetch.css'; ?>
    </style>
</head>
...
<body>
    ...
    <header class="topbar">
        <a class="topbar-brand" href="">
            ...
            <div class="topbar-name">Salesforce Device Sync <span>Salesforce → PostgreSQL</span></div>
        </a>
        <div class="topbar-actions">
            <a href="list.php" class="topbar-btn"><i class="fas fa-list"></i> Distribution List</a>
            <button class="topbar-btn primary" onclick="window.open('list.php?api=1&action=records','_blank')"><i class="fas fa-code"></i> API</button>
    ...
    <div class="page-wrap">
        ...
        <div class="stats-row">
            ...
            <div class="stat-card">
                ...
                <div>
                    <div class="stat-val" id="statLastSyncTime">
                        <?php if (isset($tableStats['last_synced'])): ?>
                            <?= date('H:i:s', strtotime($tableStats['last_synced'])) ?>
                        <?php else: ?>
                            —
                        <?php endif; ?>
                    </div>
                    <div class="stat-lbl" id="statLastSyncDate">
                        <?php if (isset($tableStats['last_synced'])): ?>
                            <?= date('d M Y', strtotime($tableStats['last_synced'])) ?>
                        <?php else: ?>
                            Last Sync
                        <?php endif; ?>
                    </div>
                    <small style="font-size:.75rem;color:var(--ink3);">
    ...
    `;
        ...
        function updateStats(stats) {
            ...
            setText('statTotal', (stats.total_records || 0).toLocaleString());
            setText('statDeviceTypes', (stats.unique_device_types || 0).toLocaleString());
            setText('statCountries', (stats.unique_countries || 0).toLocaleString());

            // Update Last Sync
            if (stats.last_synced) {
                const date = new Date(stats.last_synced);
                setText('statLastSyncTime', date.toTimeString().substring(0, 8)); // HH:MM:SS
                ...
                setText('statLastSyncDate', date.toLocaleDateString('en-GB', options)); // "15 May 2026"
            }
        }
