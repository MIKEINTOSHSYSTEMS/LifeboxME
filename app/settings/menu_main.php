<?php
global $runnerMenus;
$runnerMenus['main'] = array(
	'name' => 'main',
	'id' => 'main',
	'treeLike' => true,
	'root' => array(
		'id' => '',
		'parent' => '',
		'children' => array( 
			array(
				'id' => '73',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '-------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '72',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'table' => 'Dashboard',
						'type' => 6 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => 'mypage.htm',
					'params' => '',
					'pageId' => '',
					'itemType' => 2,
					'linkType' => 0,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-dashboard',
					'iconStyle' => 3,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => 'dashboard',
					'table' => 4701 
				) 
			),
			array(
				'id' => '32',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '-------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '37',
				'parent' => '0',
				'children' => array( 
					array(
						'id' => '27',
						'parent' => '37',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Training Summary',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-list-alt',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4641 
						) 
					),
					array(
						'id' => '165',
						'parent' => '37',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'device_distributions_report',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-th-list',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'table' => 19091,
							'pageType' => 'report' 
						) 
					),
					array(
						'id' => '26',
						'parent' => '37',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Surgical Case Summary',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-scribd',
							'iconStyle' => 2,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4644 
						) 
					),
					array(
						'id' => '25',
						'parent' => '37',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Device Distribution Summary',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-mobile-phone',
							'iconStyle' => 3,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4643 
						) 
					),
					array(
						'id' => '24',
						'parent' => '37',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Clean Cut Implementation Summary',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-hand-scissors',
							'iconStyle' => 1,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4639 
						) 
					),
					array(
						'id' => '68',
						'parent' => '37',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => '{04AFFBE6-86C0-47b0-ADD3-BA7FA19CA6FC}',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => 'WebReports',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-pie-chart',
							'iconStyle' => 3,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'WebReports' 
						) 
					) 
				),
				'data' => array(
					'name' => array(
						'text' => 'Reports & Summaries',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 0,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-th-list',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '55',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '-------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '33',
				'parent' => '0',
				'children' => array( 
					array(
						'id' => '20',
						'parent' => '33',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.training_sessions',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-slideshare',
							'iconStyle' => 2,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4630 
						) 
					),
					array(
						'id' => '90',
						'parent' => '33',
						'children' => array( 
							array(
								'id' => '92',
								'parent' => '90',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Training Dashboard',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => 'all_in_one_training',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-dashboard',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'dashboard',
									'table' => 4701 
								) 
							),
							array(
								'id' => '91',
								'parent' => '90',
								'children' => array( 
									array(
										'id' => '88',
										'parent' => '91',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'table' => 'aio_training_tracking Chart',
												'type' => 6 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-bar-chart',
											'iconStyle' => 3,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'chart',
											'table' => 4717 
										) 
									),
									array(
										'id' => '89',
										'parent' => '91',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'table' => 'aio_training_tracking Chart by sex',
												'type' => 6 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-bar-chart-o',
											'iconStyle' => 3,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'chart',
											'table' => 4720 
										) 
									) 
								),
								'data' => array(
									'name' => array(
										'text' => 'Charts',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 0,
									'linkType' => 2,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-chart-bar',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => '',
									'table' => null 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Training Dashboard & Charts',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-chart-bar',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '44',
						'parent' => '33',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.training_participants',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-street-view',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4659 
						) 
					),
					array(
						'id' => '71',
						'parent' => '33',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'AIO Training Tracking',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-dedent',
							'iconStyle' => 3,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4700 
						) 
					),
					array(
						'id' => '58',
						'parent' => '33',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => '-------',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 1,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 0,
							'iconName' => '',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '160',
						'parent' => '33',
						'children' => array( 
							array(
								'id' => '157',
								'parent' => '160',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Courses',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-swatchbook',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'table' => 10184,
									'pageType' => 'list' 
								) 
							),
							array(
								'id' => '156',
								'parent' => '160',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Course Users',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-users',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'table' => 10062,
									'pageType' => 'list' 
								) 
							),
							array(
								'id' => '155',
								'parent' => '160',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Course Analytics',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-chalkboard-teacher',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'table' => 9999,
									'pageType' => 'list' 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Lifebox Learning Network',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-battle-net',
							'iconStyle' => 2,
							'showIconType' => 1,
							'linkToAnotherApp' => false 
						) 
					),
					array(
						'id' => '50',
						'parent' => '33',
						'children' => array( 
							array(
								'id' => '46',
								'parent' => '50',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.training_attendance',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-check-square',
									'iconStyle' => 1,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4665 
								) 
							),
							array(
								'id' => '51',
								'parent' => '50',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.training_participation',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-check-circle',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4668 
								) 
							),
							array(
								'id' => '49',
								'parent' => '50',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.participant_attendance_summary',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-align-justify',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4667 
								) 
							),
							array(
								'id' => '48',
								'parent' => '50',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.participant_attendance_detail',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-calendar',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4664 
								) 
							),
							array(
								'id' => '52',
								'parent' => '50',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Training Daily Attendance Detail',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-clock',
									'iconStyle' => 1,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4669 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Attendance',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-calendar-check',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '57',
						'parent' => '33',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => '-------',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 1,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 0,
							'iconName' => '',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '123',
						'parent' => '33',
						'children' => array( 
							array(
								'id' => '124',
								'parent' => '123',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Test Administration',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => 'lifebox_test_center',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-pencil-square',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'dashboard',
									'table' => 4701 
								) 
							),
							array(
								'id' => '126',
								'parent' => '123',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Test Takers Portal',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '/modules/test/lbq/public/login.php',
									'params' => '',
									'pageId' => 'list',
									'itemType' => 2,
									'linkType' => 1,
									'openType' => 1,
									'iconType' => 3,
									'iconName' => 'fa-external-link-square-alt',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => '',
									'table' => null 
								) 
							),
							array(
								'id' => '127',
								'parent' => '123',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => '-------',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 1,
									'linkType' => 2,
									'openType' => 0,
									'iconType' => 0,
									'iconName' => '',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => '',
									'table' => null 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Exams & Test Administration',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-newspaper',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '53',
						'parent' => '33',
						'children' => array( 
							array(
								'id' => '16',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.training_courses',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-book',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4624 
								) 
							),
							array(
								'id' => '18',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.training_leads',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-user-md',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4627 
								) 
							),
							array(
								'id' => '40',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.training_types',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-list-ol',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4648 
								) 
							),
							array(
								'id' => '45',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Training Venues',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-glass',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4663 
								) 
							),
							array(
								'id' => '19',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.training_partners',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-first-order',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4631 
								) 
							),
							array(
								'id' => '17',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.training_languages',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-language',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4625 
								) 
							),
							array(
								'id' => '39',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.training_approaches',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-random',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4649 
								) 
							),
							array(
								'id' => '42',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Training Participant Role',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-arrows',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4655 
								) 
							),
							array(
								'id' => '21',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Trainers Assignment Alloc',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-sitemap',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4633 
								) 
							),
							array(
								'id' => '15',
								'parent' => '53',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Training Session Trainers',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-group',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4623 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Training Settings',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-themeisle',
							'iconStyle' => 2,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					) 
				),
				'data' => array(
					'name' => array(
						'text' => 'Training Management',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 0,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-leanpub',
					'iconStyle' => 2,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '56',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '-------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '67',
				'parent' => '0',
				'children' => array( 
					array(
						'id' => '5',
						'parent' => '67',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.device_distributions',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-medkit',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4605 
						) 
					),
					array(
						'id' => '63',
						'parent' => '67',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.devices',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-laptop',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4687 
						) 
					),
					array(
						'id' => '69',
						'parent' => '67',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.device_distribution_log',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-table',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4695 
						) 
					),
					array(
						'id' => '70',
						'parent' => '67',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.device_inventory_changes',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-history',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4698 
						) 
					) 
				),
				'data' => array(
					'name' => array(
						'text' => 'Devices Management',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 0,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-desktop',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '31',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '-------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '112',
				'parent' => '0',
				'children' => array( 
					array(
						'id' => '95',
						'parent' => '112',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Annual Plan Tracker',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-circle-o-notch',
							'iconStyle' => 3,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4724 
						) 
					),
					array(
						'id' => '117',
						'parent' => '112',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => '-------',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 1,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 0,
							'iconName' => '',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '113',
						'parent' => '112',
						'children' => array( 
							array(
								'id' => '129',
								'parent' => '113',
								'children' => array( 
									array(
										'id' => '98',
										'parent' => '129',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'text' => 'CRI',
												'type' => 0 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-copyright',
											'iconStyle' => 0,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'list',
											'table' => 4727 
										) 
									),
									array(
										'id' => '101',
										'parent' => '129',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'text' => 'ELMA',
												'type' => 0 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-ravelry',
											'iconStyle' => 2,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'list',
											'table' => 4736 
										) 
									),
									array(
										'id' => '96',
										'parent' => '129',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'text' => 'Beginnings Fund',
												'type' => 0 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-bold',
											'iconStyle' => 0,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'list',
											'table' => 4725 
										) 
									),
									array(
										'id' => '130',
										'parent' => '129',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'text' => 'Cathedral',
												'type' => 0 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-crosshairs',
											'iconStyle' => 0,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'list',
											'table' => 4765 
										) 
									),
									array(
										'id' => '131',
										'parent' => '129',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'text' => 'IZUMI',
												'type' => 0 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-hand-lizard',
											'iconStyle' => 1,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'list',
											'table' => 4768 
										) 
									),
									array(
										'id' => '132',
										'parent' => '129',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'text' => 'Smile Train (ST)',
												'type' => 0 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-smile',
											'iconStyle' => 1,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'list',
											'table' => 4770 
										) 
									),
									array(
										'id' => '133',
										'parent' => '129',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'text' => 'WHI',
												'type' => 0 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-female',
											'iconStyle' => 0,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'list',
											'table' => 4771 
										) 
									),
									array(
										'id' => '105',
										'parent' => '129',
										'children' => array( 
											 
										),
										'data' => array(
											'name' => array(
												'text' => 'Others Programs',
												'type' => 0 
											),
											'comments' => array(
												'text' => '',
												'type' => 0 
											),
											'style' => '',
											'href' => 'mypage.htm',
											'params' => '',
											'pageId' => '',
											'itemType' => 2,
											'linkType' => 0,
											'openType' => 0,
											'iconType' => 3,
											'iconName' => 'fa-opera',
											'iconStyle' => 2,
											'showIconType' => 1,
											'linkToAnotherApp' => false,
											'pageType' => 'list',
											'table' => 4744 
										) 
									) 
								),
								'data' => array(
									'name' => array(
										'text' => 'Programs',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 0,
									'linkType' => 2,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-product-hunt',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => '',
									'table' => null 
								) 
							),
							array(
								'id' => '100',
								'parent' => '113',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Development',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-dashcube',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4732 
								) 
							),
							array(
								'id' => '102',
								'parent' => '113',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Governance',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-google-plus',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4737 
								) 
							),
							array(
								'id' => '121',
								'parent' => '113',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Finance',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-dollar',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4761 
								) 
							),
							array(
								'id' => '122',
								'parent' => '113',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'HR',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-users',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4763 
								) 
							),
							array(
								'id' => '104',
								'parent' => '113',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Operations',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-opera',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4742 
								) 
							),
							array(
								'id' => '97',
								'parent' => '113',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Communications',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-compress',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4728 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Tracking Groups',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-quote-right',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '115',
						'parent' => '112',
						'children' => array( 
							array(
								'id' => '109',
								'parent' => '115',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Activities Summary',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-th',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4752 
								) 
							),
							array(
								'id' => '111',
								'parent' => '115',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'KPIs Tracking View',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-external-link-square',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4756 
								) 
							),
							array(
								'id' => '110',
								'parent' => '115',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Dashboard Summary',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-table',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4751 
								) 
							),
							array(
								'id' => '118',
								'parent' => '115',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Unified Activities',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-universal-access',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4758 
								) 
							),
							array(
								'id' => '120',
								'parent' => '115',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => '-------',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 1,
									'linkType' => 2,
									'openType' => 0,
									'iconType' => 0,
									'iconName' => '',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => '',
									'table' => null 
								) 
							),
							array(
								'id' => '103',
								'parent' => '115',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'KPIs On Track Adj',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-external-link',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4739 
								) 
							),
							array(
								'id' => '99',
								'parent' => '115',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Dashboard Summary Adj',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-audio-description',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4731 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Tracking Summary',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-th-list',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '114',
						'parent' => '112',
						'children' => array( 
							array(
								'id' => '108',
								'parent' => '114',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Strategic Areas',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-bullseye',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4747 
								) 
							),
							array(
								'id' => '107',
								'parent' => '114',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Status',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-check-circle',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4746 
								) 
							),
							array(
								'id' => '106',
								'parent' => '114',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Workflow',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-road',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4745 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Activity Tracking Setting',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-ge',
							'iconStyle' => 3,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					) 
				),
				'data' => array(
					'name' => array(
						'text' => 'Activity Tracker',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 0,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-stack-exchange',
					'iconStyle' => 2,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '143',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '-------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '141',
				'parent' => '0',
				'children' => array( 
					array(
						'id' => '140',
						'parent' => '141',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'PMI Summary',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-scribd',
							'iconStyle' => 2,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4785 
						) 
					),
					array(
						'id' => '148',
						'parent' => '141',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Data Values',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-superscript',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4793 
						) 
					),
					array(
						'id' => '137',
						'parent' => '141',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Indicator Actuals',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-info',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4778 
						) 
					),
					array(
						'id' => '142',
						'parent' => '141',
						'children' => array( 
							array(
								'id' => '153',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Indicator Groups',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-layer-group',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4799 
								) 
							),
							array(
								'id' => '135',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Data Elements',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-elementor',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4776 
								) 
							),
							array(
								'id' => '136',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Datasets',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-dailymotion',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4777 
								) 
							),
							array(
								'id' => '138',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Indicator Targets',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-bullseye',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4779 
								) 
							),
							array(
								'id' => '139',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Indicators',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-info-circle',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4782 
								) 
							),
							array(
								'id' => '166',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Data Sources',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-sourcetree',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'table' => 19319,
									'pageType' => 'list' 
								) 
							),
							array(
								'id' => '167',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Data Source Details',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-sort-amount-desc',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'table' => 19341,
									'pageType' => 'list' 
								) 
							),
							array(
								'id' => '151',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => '-------',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 1,
									'linkType' => 2,
									'openType' => 0,
									'iconType' => 0,
									'iconName' => '',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => '',
									'table' => null 
								) 
							),
							array(
								'id' => '149',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Aggregation Type',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-campground',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4796 
								) 
							),
							array(
								'id' => '152',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Indicator Types',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-stream',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4798 
								) 
							),
							array(
								'id' => '150',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Domain Type',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-dochub',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4797 
								) 
							),
							array(
								'id' => '146',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => '-------',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 1,
									'linkType' => 2,
									'openType' => 0,
									'iconType' => 0,
									'iconName' => '',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => '',
									'table' => null 
								) 
							),
							array(
								'id' => '147',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'PMI Calculation Jobs',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-briefcase',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4790 
								) 
							),
							array(
								'id' => '134',
								'parent' => '142',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'PMI-Calculation Log',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-calculator',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4772 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'PMI Settings',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-gears',
							'iconStyle' => 3,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					) 
				),
				'data' => array(
					'name' => array(
						'text' => 'Lifebox PMI',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 0,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-balance-scale',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '30',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '-------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '36',
				'parent' => '0',
				'children' => array( 
					array(
						'id' => '14',
						'parent' => '36',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.surgical_cases',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-stethoscope',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4619 
						) 
					),
					array(
						'id' => '3',
						'parent' => '36',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.clean_cut_implementations',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-cut',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4603 
						) 
					),
					array(
						'id' => '6',
						'parent' => '36',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.diagnoses',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-odnoklassniki',
							'iconStyle' => 2,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4604 
						) 
					),
					array(
						'id' => '12',
						'parent' => '36',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.procedures',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-bed',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4612 
						) 
					),
					array(
						'id' => '8',
						'parent' => '36',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.follow_ups',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-user-times',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4609 
						) 
					),
					array(
						'id' => '23',
						'parent' => '36',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'table' => 'public.ward_rounds',
								'type' => 6 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-retweet',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4637 
						) 
					),
					array(
						'id' => '116',
						'parent' => '36',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => '-------',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 1,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 0,
							'iconName' => '',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '34',
						'parent' => '36',
						'children' => array( 
							array(
								'id' => '1',
								'parent' => '34',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.antibiotics',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-deviantart',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4598 
								) 
							),
							array(
								'id' => '2',
								'parent' => '34',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.case_antibiotics',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-xing',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4600 
								) 
							),
							array(
								'id' => '22',
								'parent' => '34',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.ward_antibiotics',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-xing-square',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4636 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Antibiotics',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-eyedropper',
							'iconStyle' => 3,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					) 
				),
				'data' => array(
					'name' => array(
						'text' => 'Cases & Procedures',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 0,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-folder-open',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '29',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '-------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '75',
				'parent' => '0',
				'children' => array( 
					array(
						'id' => '77',
						'parent' => '75',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Lifebox DHIS2 Org Units',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-cubes',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4704 
						) 
					),
					array(
						'id' => '83',
						'parent' => '75',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Lifebox DHIS2 Data Items',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-dashcube',
							'iconStyle' => 2,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4712 
						) 
					),
					array(
						'id' => '87',
						'parent' => '75',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Lifebox DHIS2 Analytics Data',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-cube',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4715 
						) 
					),
					array(
						'id' => '76',
						'parent' => '75',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => '-------',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 1,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 0,
							'iconName' => '',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					) 
				),
				'data' => array(
					'name' => array(
						'text' => 'DHIS2',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 0,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-object-ungroup',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '164',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '---------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false 
				) 
			),
			array(
				'id' => '54',
				'parent' => '0',
				'children' => array( 
					array(
						'id' => '79',
						'parent' => '54',
						'children' => array( 
							array(
								'id' => '161',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'LBLN Data Fetcher',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-angle-double-down',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'table' => 10452,
									'pageType' => 'list' 
								) 
							),
							array(
								'id' => '163',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'LBLN - API Tester',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => 'lbln_api_tester',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-network-wired',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'table' => 10452,
									'pageType' => 'list' 
								) 
							),
							array(
								'id' => '158',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Fetch Logs',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-list-alt',
									'iconStyle' => 1,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'table' => 10283,
									'pageType' => 'list' 
								) 
							),
							array(
								'id' => '159',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'Fetch Sessions',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-warehouse',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'table' => 10340,
									'pageType' => 'list' 
								) 
							),
							array(
								'id' => '81',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => '-------',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 1,
									'linkType' => 2,
									'openType' => 0,
									'iconType' => 0,
									'iconName' => '',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => '',
									'table' => null 
								) 
							),
							array(
								'id' => '78',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'DHIS2 OrgUnits Fetcher',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => 'list',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-cloud-download',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4707 
								) 
							),
							array(
								'id' => '84',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'DHIS2 DataItems Fetcher',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => 'dhis2_dataitems_fetcher',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-cloud-download',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4707 
								) 
							),
							array(
								'id' => '86',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'DHIS2 Analytics Fetcher Settings',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-cloud-download',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4711 
								) 
							),
							array(
								'id' => '93',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => 'DHIS2 Automated Data Fetcher CronJob',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => 'dhis2_automated_data_fetcher_cronjob',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-tasks',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4707 
								) 
							),
							array(
								'id' => '85',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => '-------',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => '',
									'params' => '',
									'pageId' => '',
									'itemType' => 1,
									'linkType' => 2,
									'openType' => 0,
									'iconType' => 0,
									'iconName' => '',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => '',
									'table' => null 
								) 
							),
							array(
								'id' => '74',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => '[BETA] Lifebox DHIS2 OrgUnits - API',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-subscript',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4705 
								) 
							),
							array(
								'id' => '82',
								'parent' => '79',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'text' => '[BETA] Lifebox DHIS2 DataItems - API',
										'type' => 0 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-subscript',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4708 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'Data Fetchers',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-cloud',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					),
					array(
						'id' => '162',
						'parent' => '54',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => '---------',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 1,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 0,
							'iconName' => '',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false 
						) 
					),
					array(
						'id' => '35',
						'parent' => '54',
						'children' => array( 
							array(
								'id' => '7',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.facilities',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-hospital',
									'iconStyle' => 1,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4608 
								) 
							),
							array(
								'id' => '13',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.regions',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-location-arrow',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4613 
								) 
							),
							array(
								'id' => '4',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.countries',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-globe',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4602 
								) 
							),
							array(
								'id' => '9',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.languages',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-language',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4614 
								) 
							),
							array(
								'id' => '11',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.partners',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-product-hunt',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4601 
								) 
							),
							array(
								'id' => '66',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.donors',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-diamond',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4692 
								) 
							),
							array(
								'id' => '38',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.programs',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-get-pocket',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4647 
								) 
							),
							array(
								'id' => '144',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.period_types',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-times-rectangle',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4789 
								) 
							),
							array(
								'id' => '62',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.months',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-calendar',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4684 
								) 
							),
							array(
								'id' => '41',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.quarters',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-quora',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4651 
								) 
							),
							array(
								'id' => '119',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.years',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-yahoo',
									'iconStyle' => 2,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4759 
								) 
							),
							array(
								'id' => '43',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.sex',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-transgender-alt',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4658 
								) 
							),
							array(
								'id' => '145',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.unit_of_measurement',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-balance-scale',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4791 
								) 
							),
							array(
								'id' => '10',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.lifebox_staff',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-id-badge',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4617 
								) 
							),
							array(
								'id' => '64',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.payment_methods',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-money',
									'iconStyle' => 3,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4688 
								) 
							),
							array(
								'id' => '65',
								'parent' => '35',
								'children' => array( 
									 
								),
								'data' => array(
									'name' => array(
										'table' => 'public.transaction_types',
										'type' => 6 
									),
									'comments' => array(
										'text' => '',
										'type' => 0 
									),
									'style' => '',
									'href' => 'mypage.htm',
									'params' => '',
									'pageId' => '',
									'itemType' => 2,
									'linkType' => 0,
									'openType' => 0,
									'iconType' => 3,
									'iconName' => 'fa-trademark',
									'iconStyle' => 0,
									'showIconType' => 1,
									'linkToAnotherApp' => false,
									'pageType' => 'list',
									'table' => 4690 
								) 
							) 
						),
						'data' => array(
							'name' => array(
								'text' => 'LB Variables',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => '',
							'params' => '',
							'pageId' => '',
							'itemType' => 0,
							'linkType' => 2,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-gears',
							'iconStyle' => 3,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => '',
							'table' => null 
						) 
					) 
				),
				'data' => array(
					'name' => array(
						'text' => 'Settings',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 0,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-gear',
					'iconStyle' => 3,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '28',
				'parent' => '0',
				'children' => array( 
					 
				),
				'data' => array(
					'name' => array(
						'text' => '-------',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 1,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 0,
					'iconName' => '',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			),
			array(
				'id' => '61',
				'parent' => '0',
				'children' => array( 
					array(
						'id' => '59',
						'parent' => '61',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Users Audit',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-trello',
							'iconStyle' => 2,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4683 
						) 
					),
					array(
						'id' => '128',
						'parent' => '61',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Visitors Analytics',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => 'lifebox_visitors_analytics',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-line-chart',
							'iconStyle' => 3,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'dashboard',
							'table' => 4701 
						) 
					),
					array(
						'id' => '94',
						'parent' => '61',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'SMTP Settings',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-paper-plane',
							'iconStyle' => 1,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4721 
						) 
					),
					array(
						'id' => '60',
						'parent' => '61',
						'children' => array( 
							 
						),
						'data' => array(
							'name' => array(
								'text' => 'Table & Session  Locking',
								'type' => 0 
							),
							'comments' => array(
								'text' => '',
								'type' => 0 
							),
							'style' => '',
							'href' => 'mypage.htm',
							'params' => '',
							'pageId' => '',
							'itemType' => 2,
							'linkType' => 0,
							'openType' => 0,
							'iconType' => 3,
							'iconName' => 'fa-unlock-alt',
							'iconStyle' => 0,
							'showIconType' => 1,
							'linkToAnotherApp' => false,
							'pageType' => 'list',
							'table' => 4686 
						) 
					) 
				),
				'data' => array(
					'name' => array(
						'text' => 'System Administration',
						'type' => 0 
					),
					'comments' => array(
						'text' => '',
						'type' => 0 
					),
					'style' => '',
					'href' => '',
					'params' => '',
					'pageId' => '',
					'itemType' => 0,
					'linkType' => 2,
					'openType' => 0,
					'iconType' => 3,
					'iconName' => 'fa-user-shield',
					'iconStyle' => 0,
					'showIconType' => 1,
					'linkToAnotherApp' => false,
					'pageType' => '',
					'table' => null 
				) 
			) 
		),
		'data' => array(
			'name' => array(
				'text' => '',
				'type' => 0 
			),
			'comments' => array(
				'text' => '',
				'type' => 0 
			),
			'style' => '',
			'href' => '',
			'params' => '',
			'pageId' => '',
			'itemType' => 0,
			'linkType' => 2,
			'openType' => 0,
			'iconType' => 0,
			'iconName' => '',
			'iconStyle' => 0,
			'showIconType' => 1,
			'linkToAnotherApp' => false 
		) 
	) 
);
?>